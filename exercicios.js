...
function listarCurvaRotativida() {
...
function reporEstoque(rotatividade, curva) {
  let produtoEstoque = produtos.filter(
    (produto) =>
      produto.rotatividade == rotatividade && produto.curva_abc == curva,
  );
...
function promocao() {
  let descontoproduto = produtos
    .filter(
      (produto) =>
        produto.curva_abc === "C" && produto.rotatividade === "baixa",
    )
